...
export default function LayoutCanvas({ nodes, setNodes, selectedId, onSelect, relations = [], justAdded, onDelete, onCopy, onPaste, onUndo, onRedo, canUndo = false, canRedo = false, onHistorySave, isParsing = false }: LayoutCanvasProps) {
  ...
  const handleMouseDownNode = (e: React.MouseEvent, id: string) => {
    ...
    const up = () => {
      ...
      // We need to read from the actual current state, not closure
      // Use a ref or read from the latest state
      setNodes(currentNodes => {
        const finalNodes = currentNodes.map(n => (n.id === currentId ? { ...n, x: snap(n.x), y: snap(n.y) } : n));
        const changed = JSON.stringify(startNodes) !== JSON.stringify(finalNodes);
        if (changed) {
          onHistorySave && onHistorySave(finalNodes);
          return finalNodes;
        }
        return currentNodes; // No change, return as-is
      });
  ...
  const handleMouseDownHandle = (e: React.MouseEvent, id: string) => {
    ...
    const up = () => {
      ...
      setNodes(currentNodes => {
        const finalNodes = currentNodes.map(n => (n.id === currentId ? { ...n, w: Math.max(20, snap(n.w)), h: Math.max(20, snap(n.h)) } : n));
        const changed = JSON.stringify(startNodes) !== JSON.stringify(finalNodes);
        if (changed) {
          onHistorySave && onHistorySave(finalNodes);
          return finalNodes;
      ...
      <div className="relative">
        ...
        <g transform={`translate(${pan.x},${pan.y}) scale(${scale})`}>
          ...
          {nodes.map((n, nodeIdx) => {
            ...
            // Detect overlaps: check if this box overlaps with any box that comes after it (rendered on top)
            // If overlapped, make this box bigger to show all text
            let adjustedW = n.w;
            let adjustedH = n.h;
            ...
            for (let i = nodeIdx + 1; i < nodes.length; i++) {
              ...
              const overlapX = !(n.x + n.w < otherNode.x || n.x > otherNode.x + otherNode.w);
              const overlapY = !(n.y + n.h < otherNode.y || n.y > otherNode.y + otherNode.h);
              
              if (overlapX && overlapY) {
                hasOverlap = true;
                // Calculate how much bigger we need to make this box
                // Expand to ensure both boxes' text is visible
                const neededW = Math.max(adjustedW, otherNode.x + otherNode.w - n.x + 20);
                const neededH = Math.max(adjustedH, otherNode.y + otherNode.h - n.y + 20);
                adjustedW = Math.max(adjustedW, neededW);
                adjustedH = Math.max(adjustedH, neededH);
              }
            }
            ...
            <g key={n.id} transform={`translate(${n.x},${n.y})`}>
              ...
              {isJustAdded && (
                <rect
                  x={-4}
                  y={-4}
                  width={(hasOverlap ? adjustedW : n.w) + 8}
                  height={(hasOverlap ? adjustedH : n.h) + 8}
                  rx={8}
                  fill="none"
                  stroke="#3b82f6"
                  strokeWidth="2"
              ...
              <rect
                x={0}
                y={0}
                width={hasOverlap ? adjustedW : n.w}
                height={hasOverlap ? adjustedH : n.h}
                rx={8}
              ...
              />
              {
                ...
                (() => {
                  const displayW = hasOverlap ? adjustedW : n.w;
                  const displayH = hasOverlap ? adjustedH : n.h;
                  const base = Math.max(10, Math.min(24, Math.floor(displayH / 4)));
                  const labelText = n.label || n.type;
                  const countText = n.count ? `×${n.count}` : '';
                  
                  // Estimate chars per line for label wrapping
                  const maxChars = Math.max(4, Math.floor((displayW - 12) / (base * 0.55)));
                  ...
                  const maxLines = Math.max(1, Math.floor((displayH / 2 - 8) / (base + 2)));
                  const labelLines = wrapped.slice(0, maxLines);
                  const textX = displayW / 2; // Center horizontally
              ...
              {(isSelected || isJustAdded) && (
                <rect
                  x={(hasOverlap ? adjustedW : n.w) - 10}
                  y={(hasOverlap ? adjustedH : n.h) - 10}
                  width={10}
                  height={10}
                  rx={2}
